<script setup>
import liff from "@line/liff";
import { reactive } from "vue";
import { Image as VanImage, Button } from "vant";
import router from "../router";

const userProfile = reactive({
  user_name: "",
  img_proflie: "",
  message: "",
});
const clientAPIs = reactive({
  language: "",
  version: "",
  in_client: "",
  logged_in: "",
  os: "",
  line_version: "",
  context: "",
  paramID: "",
});

const getClientAPI = async () => {
  clientAPIs.language = await liff.getLanguage();
  clientAPIs.version = await liff.getVersion();
  clientAPIs.in_client = await liff.isInClient();
  clientAPIs.logged_in = await liff.isLoggedIn();
  clientAPIs.os = await liff.getOS();
  clientAPIs.line_version = await liff.getLineVersion();
  //
  clientAPIs.context = await liff.getContext();
  console.log(liff.getContext(), "liff.getContext()");
  console.log(clientAPIs, "clientAPIs");
  const urlParams = new URLSearchParams(location.search);
  for (const [key, value] of urlParams) {
    clientAPIs.paramID = value;
  }
};
getClientAPI();

liff
  .getProfile()
  .then((profile) => {
    alert("success get profile");
    console.log(profile, "profile");
    userProfile.userName = profile.displayName;
    userProfile.img_proflie = profile.pictureUrl;
    userProfile.message = profile.statusMessage;
  })
  .catch((err) => {
    alert("error");
    console.log("error", err);
  });

const handlBack = () => {
  router.push({ name: "home-page" });
};
</script>

<template>
  <div class="contener">
    <div>
      <b style="font-size: 50px; color: white">Line Profile {{}}</b>
    </div>
    <div class="card-profile">
      <div style="margin: 10px 50px 0px 50px">
        <van-image
          round
          width="10rem"
          height="10rem"
          :src="
            clientAPIs.paramID
              ? 'data:image/jpeg;base64,'
              : userProfile.img_proflie
          "
          class="img-profile"
        />
      </div>
      <div>
        Name : {{ clientAPIs.paramID ? "พี่โต" : userProfile.userName }}
      </div>
      <div>
        Message :
        {{ clientAPIs.paramID ? "พี่โตถูกใจสิ่งนี้" : userProfile.message }}
      </div>
      <div>Language : {{ clientAPIs.paramID ? "en" : clientAPIs.language }}</div>
      <div>Version : {{ clientAPIs.paramID ? " - " : clientAPIs.version }}</div>
      <div>os : {{ clientAPIs.paramID ? " - " : clientAPIs.os }}</div>
      <div>
        line_version :
        {{ clientAPIs.line_version ? " - " : clientAPIs.line_version }}
        {{ console.log(clientAPIs.line_version ,'clientAPIs.line_version ')
         }}
      </div>
      <Button plain type="danger" @click="handlBack" class="button-back"
        >Back</Button
      >
    </div>
  </div>
</template>

<style scoped>
.card-profile {
  display: flex;
  flex-direction: column;
  row-gap: 20px;
  background-color: white;
  padding: 10%;
  border-radius: 30px;
  align-items: center;
  height: calc(100vh - 45%);
}
.contener {
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: center;
  width: 100%;
  height: 100vh;
}
.button-back {
  width: 100%;
  margin-top: 30px;
  border-radius: 50px;
}
.img-profile {
  border: 5px solid transparent;
  border-color: rgb(241, 193, 233);
}
</style>
